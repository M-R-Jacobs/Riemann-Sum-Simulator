...
Here is some information about each method. This does not necessarily reflect what is done in the code, but serves as the mathematical basis:

**1. Reimann Sums (Left, Right, and Midpoint)**

Riemann sums approximate areas under curves using $n$ number of adjacent rectangles, with one corner of each rectangle placed on the curve $f(x)$ as it varies (the height of a given rectangle is the value of $f(x)$ at that $x$-value.)
...
$M_i$ now is simply $a+i\Delta x$ as the right endpoint and corresponding function height is located at the interval start value $a$ plus 1 subinterval. We need to start there, so we add one $\Delta x$ to start. Everything else is the same. Clearly, computationally, this is a little easier the left Riemann sums. However, both left and right sums have significant issues in estimating the areas under increasing and decreasing slopes without under-estimating or over-estimating.
...
A trapezoid is a quadrilateral with a pair of parallel sides ($b_1$ and $b_2$) that have different lengths connected by two sides that are not parallel and do not necessarily have an angular or symmetrical relationship to one another. If they did, it would be a "regular trapezoid" which we do not use here. In the area approximation under curves, the parallel bases are oriented vertically and act as the height of the function at two points separated by one subinterval - $f(i=k)$ and $f(i=k+1)$. One of the two remaining sides acts as the perpendicular distance between them (the "height" or altitude $h$ of the trapezoid itself) - this is $\Delta x=\frac{b-a}{n}$. The fourth is merely drawn between the two endpoints of the heights and is not important in the calculation of the area, which for a trapezoid is:
