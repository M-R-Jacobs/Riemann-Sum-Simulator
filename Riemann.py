...
for i, ax in enumerate(axes[:5]):   # only the first 5 axes are used for the plots, the last one is for data
    ...
    ax.set_ylim(0, 5)
    ax.set_xticks([1,2,3,4])    # integer ticks only
...
def redraw(n):  # handles all n updates, and will be called by the radio button callback as well
    ...
    for i, method in enumerate(METHODS):
        ...
        y_pct = y_top - 0.055           # % errore line
        y_bar = y_pct - 0.04            # bar = below % error line
        ...
        if clipped:
            ax_error.annotate(
                "", 
                xy=(bar_left + fill_width + 0.06, y_bar - bar_thickness / 2),
                xytext=(bar_left + fill_width, y_bar - bar_thickness / 2),
